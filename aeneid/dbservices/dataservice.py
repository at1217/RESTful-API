...
def get_by_primary_key_path(table_name, key, sub_resource,
                            field_list=None,limit=None,offset=None,order_by=None,
                            commit=True):

    dt = get_data_table(table_name)
    result = dt.find_by_path_key(table_name,key,sub_resource,field_list,
...
def get_by_query_from_h(table_name,child_resources, template,field_list):
    try:
        dt = get_data_table(table_name)
        result = dt.find_by_path_template(table_name, child_resources=child_resources,
                                          template=template,field_list=field_list,
...
def get_by_new_template(table_name, table2_name, template, field_list=None, limit=None, offset=None, order_by=None, commit=True):

    dt = get_data_table(table_name)
    result = dt.find_by_template(template, field_list, limit, offset, order_by, commit)
    return result.get_rows()
#def get_related(table_name,row)
#def update_by_key(table_name,key,related_name,new_r)
#def get_by_primary_key_string()

def get_by_primary_key_new(parent, child, primary_key):
    dt. get
    result = dt.join_two(parent, child, primary_key)
